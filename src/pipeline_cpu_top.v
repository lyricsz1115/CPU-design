module pipeline_cpu_top #(
    ...
    imem #(.INIT_FILE(INIT_FILE)) u_imem(.addr(pc_value), .inst(inst_if));
    ...
    control u_control(.opcode(id_opcode), .branch(id_branch), .jal(id_jal), .mem_read(id_mem_read), .mem_to_reg(id_mem_to_reg), .alu_op(id_alu_op), .mem_write(id_mem_write), .alu_src(id_alu_src), .reg_write(id_reg_write));
    ...
    id_ex_reg u_id_ex(
        ...
        .pc_in(if_id_pc), .reg_data1_in(id_reg_data1), .reg_data2_in(id_reg_data2), .imm_in(id_imm),
